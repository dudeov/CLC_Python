...
def upload(dev_cred, fn_prim, fn_sec):
  ...
  print('#'*30)
  child = pexpect.spawn('sftp anonymous@sftp.juniper.net')
  try:
    ...
    child.expect('sftp>', timeout = 900)
    child.sendline('put /root/jtac/' + dev_cred['device_hn'] + '/' + fn_prim)
    child.expect('sftp>', timeout = 900)
    child.sendline('put /root/jtac/' + dev_cred['device_hn'] + '/' + fn_sec)
    ...
    if 'Permission denied' in download_str_1 or 'Permission denied' in download_str_2:
      print('###########\n\n\n\n\nSomething went wrong! Files was not copied \n\n\n########')
      sys.exit(1)
    else: 
